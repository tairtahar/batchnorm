...
def model_selection(network, input_shape, output_size, epsilon, window_size):
    ...
    if network == 'lenet':
        ...
        lenet_model = LeNetBN1(input_shape=input_shape, output_size=output_size, epsilon=epsilon, window=window_size)
    elif network == 'lenet_bn2':
        print("Chosen network is LeNet with Batchnorm on first + second convolution layers")
        lenet_model = LeNetBN2(input_shape=input_shape, output_size=output_size, epsilon=epsilon, window=window_size)
        ...
        lenet_model = LeNetFCBN1(input_shape=input_shape, output_size=output_size, epsilon=epsilon, window=window_size)
        ...
        lenet_model = LeNetFCBN2(input_shape=input_shape, output_size=output_size, epsilon=epsilon, window=window_size)
    else:
...
def model_execution(network, data, output_size, batch_size, optimizer, epochs, epsilon, window_size, verbose):
    ...
    lenet_model = model_selection(network, input_shape, output_size, epsilon, window_size)
